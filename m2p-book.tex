...
Let $(\Omega_1,\F,P_1)$ and $(\Omega_2,\G,P_2)$ be two probability spaces. We can then construct the product probability measure $P = P_1\otimes P_2$ on $(\Omega_1\times\Omega_2,\F_1 \otimes \F_2)$. The \df{joint distribution} of two random variables $X$ and $Y$ respectively on $(\Omega_1,\F,P_1)$ and $(\Omega_2,\G,P_2)$ is defined by \[
    \mu_{X,Y}(E) = P\bigl((X,Y)\in E\bigr)\quad \text{for }E\in \F\otimes \G.
\]

Generalize to $n$ random variables

\begin{fact}
    $X$ and $Y$ are independent if and 
\end{fact}
